...
namespace InventoryManagement.API.Controllers
{
    public class TransactionAPIController : ApiController
    {
        ...
        public ResponseDetail RejectFranchiseOrder(string OrderNo, string RejectReason, decimal RejectedByUserId,bool RejectCompletely)
        {
            ...
            Sql=Sql+" FROM TrnPartyOrderMain a LEFT JOIN (Select OrderNo, SUM(NetPayable) NetPayable FROM TrnBillMain WHERE OrderNo='" + OrderNo + "' GROUP BY OrderNo) b  ";
               ...
               if(SC.State==ConnectionState.Closed)
                ...
                " Select CASE WHEN Max(VoucherNo) is NULL THEN 1 ELSE Max(VoucherNo)+1 END ,Cast(Convert(varchar,Getdate(),106) as Datetime),'" + OrderTo + "','" + OrderBy + "','" + WalletAmt + "','Order " + OrderNo + " generated for product.','" + OrderNo + "','R','O','Order Generated',(Select Max(SessID) FROM " + db + "..M_SessnMaster),(Select Max(FSessID) FROM M_FiscalMaster) FROM TrnVoucher ;";
                        }
                    }
                }
                ...
                using (var entity = new InventoryEntities())
                {
                    ...
                    if (partyType == "customer")
                    {
                        billList = (from r in entity.TrnBillMains
                                    ...
                                    where (r.FType == "GC") && (r.BillDate >= date2) && (r.BillDate <= DateTime.Now.Date)
                    ...
                    else
                    {
                        billList = (from r in entity.TrnBillMains
                                    let date = r.OrderType == "T" ? date1 : date2
                                    where r.FCode == Fcode && r.FType == "M" && (r.BillDate >= date2) && (r.BillDate <= DateTime.Now.Date)
                                    select new PartyBill
                                    {
                                        BillNo = r.BillNo,
                                        UserBillNo = r.UserBillNo,
        ...
        public ResponseDetail UpdateDeliveryDetails(UpdateDeliveryDetails obj)
        {
            ...
            try
            {
                ...
                foreach (var record in obj.DeliverDetailList)
                {
                    ...
                    TrnOrderDeliveryDetail order = null;
                    using (var entity = new InventoryEntities())
                    {
                        order = (from r in entity.TrnOrderDeliveryDetails where r.BillNo == record.BillNo select r).FirstOrDefault();
                    }
                    ...
                    if (order == null)
                    {
                        ...
                        Sql += " SELECT " + MaxCode + " as SNo,SoldBy,OrderNo,OrderDate,BillNo,BillDate,CourierId,CourierName,DocWeight,DocketNo,DocketDate,DOD,DelvAddress,UserId From TrnBillMain Where FSessId='" + fessid + "' And BillNo ='" + record.BillNo + "')";

                    }
                    else
                    {
                        Sql += "Update TrnOrderDeliveryDetail";
                        Sql += " SET DocWeight= '" + record.DocWeight + "',";
                        Sql += " DocketNo= '" + record.DocketNo + "',";
                        Sql += " DocketDate= '" + (record.DocketDate != null ? ("'" + record.DocketDate + "'") : "null") + ",";
                        Sql += " DelvAddress= '" + record.DelvAddress + "'";
                        Sql += " WHERE SNo = (select max(SNo) from TrnOrderDeliveryDetail where BillNo = '" + record.BillNo + "');";
                    }
                }
        ...
        public List<PackUnpackProduct> GetPackUnpackProducts(string PackUnpack, decimal KitId, string prodID, string LoginPartyCode)
        {
            ...
            try
            {
                ...
                if (PackUnpack.ToLower() == "pack")
                {
                    ...
                    Sql += " Select b.ProdID,b.ProductName,a.Qty,0 as AvailStock FROM VisionRoots..M_KitProductDetail a,M_ProductMaster b ";
        ...
        public UpgradeID GetKitProductList(string kitId)
        {
            ...
            if (!(string.IsNullOrEmpty(kitId)))
            {
                try
                {
                    ...
                    query += " a.MRP,a.DP,b.RP as RP,b.bv as bv,'P' as ProdType,c.Barcode,a.Cv,a.PV from SJLInv..M_ProductMaster as a, VisionRoots..M_KitMaster as b,SJLInv..M_TaxMaster t, SJLInv..V#AvailProdStockBarcodes c ";
                    ...
                    query += " a.MRP,a.DP,0 as RP,0 as bv,'F' as ProdType,c.Barcode,a.Cv,a.PV from SJLInv..M_ProductMaster as a, VisionRoots..M_KitProductDetail as b, SJLInv..V#AvailProdStockBarcodes c,SJLInv..M_TaxMaster t  ";
        ...
        public List<WalletRequest> GetAllWalletRequest()
        {
                ...
                using (var entity = new InventoryEntities())
                {
                    foreach (var record in objModelList)
                    {
                        if (record.IsApproved == "Y")
                        {
                            ...
                            obj.ApproveRemark = record.ApproveRemark;
                            ...
                            string Sql = ";INSERT INTO TrnVoucher(VoucherNo,VoucherDate,DrTo,Crto,Amount,Narration,Refno,VType,BType,AccDocType,SessID,FSessID) " +
                                    " Select CASE WHEN Max(VoucherNo) is NULL THEN 1 ELSE Max(VoucherNo)+1 END ,Cast(Convert(varchar,Getdate(),106) as Datetime),'','" + PartyCode + "','" + Amount + "','Wallet credited.','WReq/" + record.ReqNo + "','R','O','Order Generated',(Select Max(SessID) FROM " + db + "..M_SessnMaster),(Select Max(FSessID) FROM M_FiscalMaster) FROM TrnVoucher ;";
                            SqlCommand cmd = new SqlCommand();
                            cmd.CommandText = Sql;
                            cmd.Connection = SC1;
                            cmd.ExecuteNonQuery();
        ...
        public ResponseDetail ReturnCourier(string BillNo,string ReturnRemark, string ReturnDate)
        {
            ResponseDetail objresponse = new ResponseDetail();
            try {
                string Sql = string.Empty;
                ...
                int MaxCode = 0;
                    ...
                    Sql += " INSERT INTO TrnOrderDeliveryDetail(SNo, SoldBy, OrderNo, OrderDate, BillNo, BillDate, CourierId, CourierName, DocWeight, DocketNo, DocketDate, DOD, DelvAddress, UserId,ReturnRemark,ReturnDate) ";
                Sql += " SELECT " + MaxCode + " as SNo,SoldBy,OrderNo,OrderDate,BillNo,BillDate,CourierId,CourierName,DocWeight,DocketNo,DocketDate,DOD,DelvAddress,UserId From TrnBillMain Where FSessId='" + fessid + "' And BillNo ='" + BillNo + "','"+ReturnRemark+"','"+ReturnDate+"'";
        ...
        public List<SalesReport> GetReturnSummary(string billNo)
        {
            List<SalesReport> listreturnsummary = new List<Entity.Common.SalesReport>();
            try {
                using (var entity = new InventoryEntities())
                {
                    listreturnsummary = (from r in entity.TrnOrderDeliveryDetails
                                         where r.BillNo == billNo
                                         select new SalesReport
                                         {
                                            ...
                                            DocketNo = r.DocketNo,
                                            DocWeight = r.DocWeight.ToString(),
                                            CourierName = r.CourierName,
                                            DelvAddress = r.DelvAddress,
                                            ReturnDate = r.ReturnDate,
                                            ReturnRemark = r.ReturnRemark                                                                                   
                                         }).ToList();
                }
            }
            catch (Exception ex)
